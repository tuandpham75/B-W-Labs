...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
	...
	public interface GameHelperListener {
		 ...
		 * 
		 * @link{GamesHelper#beginUserInitiatedSignIn . Note that not all calls to this method mean an error; it may be a result of the fact that automatic sign-in could not proceed because user interaction was required (consent dialogs). So implementations of this method should NOT display an error message unless a call to @link{GamesHelper# hasSignInError} indicates that an error indeed occurred.
		 */
		void onSignInFailed();
	...
	/*
	 * Whether user has specifically requested that the sign-in process begin. If mUserInitiatedSignIn is false, we're in the automatic sign-in attempt that we try once the Activity is started -- if true, then the user has already clicked a "Sign-In" button or something similar
	 */
